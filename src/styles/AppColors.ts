const LightColors = {
  // ─── Core (Modern Indigo / Violet Header Gradient Palette) ───
  primaryLight: '#FFFFFF',
  purple: '#4F46E5',             // Vibrant Header Indigo
  purpleShade50: '#EEF2FF',      // Indigo Silk Tint
  offerPurple: '#7C3AED',        // Vibrant Header Violet
  grayBorderSecondary: '#E2E8F0',
  grayBackground: '#F8FAFC',     // Clean Pearl Slate Surface
  grayText: '#475569',
  grayTextWeak: '#64748B',
  greenColor: '#10B981',
  graySurface: '#F1F5F9',
  skyBlue: '#2563EB',
  primaryBlack: '#0F172A',       // Slate Midnight Dark
  grayTextStrong: '#0F172A',
  dividerColor: '#E2E8F0',
  warningIconGold: '#D97706',    // Warm Amber Gold
  shadowColorString: '#0F172A',
  errorColor: '#E11D48',
  lightOrange: '#F59E0B',
  darkOrange: '#D97706',
  successGreen: '#10B981',
  purpleShade700: '#3730A3',     // Deep Indigo
  greenStatus: '#D1FAE5',
  greenBaggageText: '#059669',
  paleYellow: '#FEF3C7',

  // ─── Brand / UI ───
  white: '#FFFFFF',
  black: '#000000',
  brandPurple: '#4F46E5',        // Iconic Indigo 600
  liveGreen: '#10B981',
  firebaseOrange: '#D97706',
  greenA400: '#00E676',

  // ─── Code / Slate ───
  slate900: '#0F172A',
  slate700: '#334155',
  slate600: '#475569',
  slate500: '#64748B',
  slate400: '#94A3B8',
  slate200: '#E2E8F0',
  slate100: '#F1F5F9',
  slate50: '#F8FAFC',
  gray700: '#334155',
  gray300: '#CBD5E1',
  highlightYellow: '#FDE047',
  yellowHighlight: '#FFE44D',

  // ─── Status / Syntax ───
  teal600: '#0D9488',
  cyan600: '#0891B2',
  sky600: '#0284C7',
  blue500: '#3B82F6',
  blue600: '#2563EB',
  blue700: '#1D4ED8',
  blue800: '#1E40AF',
  indigo600: '#4F46E5',
  violet600: '#7C3AED',
  violet500: '#8B5CF6',
  purpleText: '#4F46E5',
  pink600: '#DB2777',
  rose600: '#E11D48',
  orange600: '#EA580C',
  amber500: '#F59E0B',
  amber600: '#D97706',
  amber700: '#B45309',
  amber800: '#92400E',
  green500: '#22C55E',
  green600: '#16A34A',
  green700: '#15803D',
  green800: '#166534',
  emerald500: '#10B981',
  emerald600: '#059669',
  grey600: '#64748B',
  redErrorText: '#991B1B',
  materialGreen: '#2E7D32',
  googleBlue: '#4285F4',
  googleGreen: '#34A853',
  googleOrange: '#F57C00',
  googleRed: '#E53935',
  googlePurple: '#4F46E5',
  googleTeal: '#00897B',

  // ─── NPM / Package / Tailwind Spectrum Helpers ───
  npmRed: '#CB3837',
  npmRedTint: '#CB383714',
  npmRedBorder: '#CB383733',
  npmDark: '#20232A',
  reactCyan: '#61DAFB',
  pink500: '#EC4899',
  pink400: '#F472B6',
  rose500: '#F43F5E',
  sky500: '#0EA5E9',
  sky400: '#38BDF8',
  indigo500: '#6366F1',
  purple500: '#8B5CF6',
  purple400: '#A78BFA',
  fuchsia500: '#D946EF',
  emerald400: '#34D399',
  amber400: '#FBBF24',
  orange500: '#F97316',
  red500: '#EF4444',
  teal400: '#2DD4BF',
  teal500: '#14B8A6',
  lime500: '#84CC16',
  yellow400: '#FACC15',
  yellow500: '#EAB308',
  slate800: '#1E293B',
  slate850: '#0F172A',

  // ─── Telemetry Sub-Header & Performance Ribbon (Grey Theme) ───
  telemetrySubHeaderBg: '#F1F5F9',
  telemetrySubHeaderBorder: '#E2E8F0',
  telemetrySubHeaderGradStart: '#F8FAFC',
  telemetrySubHeaderGradMid: '#F1F5F9',
  telemetrySubHeaderGradEnd: '#E2E8F0',
  telemetryPillBg: '#E2E8F0',
  telemetryTextMuted: '#64748B',

  // ─── Graphic & Spectrum Tokens ───
  teal300: '#5EEAD4',
  sky300: '#7DD3FC',
  amber200: '#FDE68A',
  amber300: '#FCD34D',
  orange400: '#FB923C',
  rose300: '#FDA4AF',
  rose400: '#FB7185',
  navy700: '#24345F',
  navy800: '#1D2A4F',
  navy900: '#121B35',
  navy950: '#080F22',
  steel500: '#52709E',
  steel600: '#4A659E',
  steel700: '#22325A',

  // ─── Bundle / Extended Spectrum ───
  reanimatedNavy: '#001A72',
  expoViolet: '#5A29E4',
  hermesPurple: '#5B44E0',
  cyan500: '#06B6D4',
  indigo400: '#818CF8',
  indigo600Alt: '#4F46E5',
  purple800: '#4338CA',
  purple900: '#312E81',
  purple50: '#EEF2FF',
  purple100: '#E0E7FF',
  purple200: '#C7D2FE',
  rose700: '#BE123C',
  red600: '#DC2626',
  red50: '#FFFBFB',
  red100: '#FEE2E2',
  amber100: '#FEF3C7',
  amber800Warm: '#B45309',
  yellow200: '#FEF08A',
  yellow800: '#854D0E',
  green100: '#DCFCE7',
  emerald100: '#D1FAE5',
  emerald700: '#047857',
  pink100: '#FCE7F3',
  sky100: '#E0F2FE',
  teal100: '#CCFBF1',
  teal700: '#0F766E',
  purple700: '#4338CA',
  indigo50: '#EEF2FF',
  gray100: '#F1F5F9',
  gray200: '#E2E8F0',
  gray500: '#64748B',
  gray600: '#475569',

  // ─── GraphQL & Special API ───
  graphqlPink: '#E10098',

  // ─── Syntax Highlighting & JSON Colors ───
  syntaxKey: '#4F46E5',
  syntaxColon: '#64748B',
  syntaxString: '#059669',
  syntaxNumber: '#D97706',
  syntaxBoolean: '#DB2777',
  syntaxNull: '#E11D48',
  syntaxBracket: '#64748B',
  syntaxPunctuation: '#94A3B8',

  // ─── Extended Status / Debug / Terminal ───
  buildSuccessGreen: '#4ADE80',
  buildFailureRed: '#F87171',
  purpleLightMuted: '#C4B5FD',
  greenSuccessDark: '#047857',
  greenSuccessBg: '#ECFDF5',
  greenSuccessBorder: '#A7F3D0',
  greenMintBorder: '#6EE7B7',
  greenMintBg: '#F0FDF4',
  mediaModalBg: '#090D16FA',
  mediaDarkTile: '#05070D',
  mediaPlayerBg: '#070C18',

  // ─── Tint Backgrounds / Borders (Theme-Aware) ───
  errorCardBg: '#FFF1F3',
  warnCardBg: '#FDFBF4',
  blueTintBg: '#EFF6FF',
  purpleTintBg: '#EEF2FF',
  purpleTint2: '#E0E7FF',
  roseBg: '#FDF2F8',
  roseBorder: '#FBCFE8',
  errorBorder: '#FECDD3',
  amberBg: '#FDF8EE',
  amberBorder: '#E8D6AC',
  emeraldBg: '#ECFDF5',
  emeraldBorder: '#A7F3D0',
  blueBg: '#EFF6FF',
  blueBorder: '#BFDBFE',
  greenBg: '#F0FDF4',
  greenBorder: '#BBF7D0',
  purpleBg: '#EEF2FF',
  purpleBorder: '#C7D2FE',
  greyBg: '#F8FAFC',
  greyBorder: '#E2E8F0',
  paperBg: '#F8FAFC',
  contentBg: '#F8FAFC',

  // ─── Toast / Glass / Chip Tokens ───
  toastBg: '#0F172AEE',
  toastBorder: '#33415588',
  mintGreenBg: '#ECFDF5',
  mintGreenBorder: '#A7F3D0',
  mintGreenText: '#059669',
  amberWarmBg: '#FFFBEB',
  amberWarmBorder: '#FDE68A',
  amberWarmText: '#92400E',
  violetSoftBg: '#EDE9FE',
  violetSoftBorder: '#DDD6FE',
  violetSoftText: '#7C3AED',
  skySoftBg: '#E0F2FE',
  skySoftBorder: '#BAE6FD',
  skySoftText: '#0284C7',

  warningAmber: '#F59E0B',
  npmRedLight: '#FF6B6B',
  red300: '#FCA5A5',
  overlayDark: 'rgba(8, 12, 22, 0.95)',
  headerGlassDark: 'rgba(15, 23, 42, 0.9)',
  badgeGlassDark: 'rgba(15, 23, 42, 0.85)',
  borderGlassLight: 'rgba(255, 255, 255, 0.08)',
  borderGlassMedium: 'rgba(255, 255, 255, 0.15)',
  whiteAlpha06: 'rgba(255, 255, 255, 0.06)',
  whiteAlpha08: 'rgba(255, 255, 255, 0.08)',
  whiteAlpha10: 'rgba(255, 255, 255, 0.1)',
  whiteAlpha12: 'rgba(255, 255, 255, 0.12)',
  whiteAlpha15: 'rgba(255, 255, 255, 0.15)',
  whiteAlpha20: 'rgba(255, 255, 255, 0.2)',
  whiteAlpha24: 'rgba(255, 255, 255, 0.24)',
  whiteAlpha30: 'rgba(255, 255, 255, 0.3)',
  whiteAlpha33: 'rgba(255, 255, 255, 0.33)',
  whiteAlpha60: 'rgba(255, 255, 255, 0.6)',

  // ─── Modal, Backdrop & Overlay Tokens ───
  modalBackdrop: 'rgba(0, 0, 0, 0.7)',
  modalBackdropLight: 'rgba(0, 0, 0, 0.5)',
  modalBackdropDark: 'rgba(0, 0, 0, 0.9)',
  glassBorder: 'rgba(255, 255, 255, 0.12)',
  glassBackground: 'rgba(255, 255, 255, 0.08)',
  glassBackgroundDark: 'rgba(0, 0, 0, 0.25)',
  overlayWhite05: 'rgba(255, 255, 255, 0.05)',
  overlayWhite10: 'rgba(255, 255, 255, 0.1)',
  overlayWhite16: 'rgba(255, 255, 255, 0.16)',
  overlayWhite20: 'rgba(255, 255, 255, 0.2)',
  overlayWhite25: 'rgba(255, 255, 255, 0.25)',
  overlayWhite30: 'rgba(255, 255, 255, 0.3)',
  overlayWhite45: 'rgba(255, 255, 255, 0.45)',
  overlayBlack05: 'rgba(0, 0, 0, 0.05)',
  overlayBlack08: 'rgba(0, 0, 0, 0.08)',
  overlayBlack10: 'rgba(0, 0, 0, 0.1)',
  overlayBlack18: 'rgba(0, 0, 0, 0.18)',
  overlayBlack20: 'rgba(0, 0, 0, 0.2)',
  overlayBlack25: 'rgba(0, 0, 0, 0.25)',
  overlayBlack30: 'rgba(0, 0, 0, 0.3)',
  overlayBlack40: 'rgba(0, 0, 0, 0.4)',
  overlayBlack50: 'rgba(0, 0, 0, 0.5)',
  overlayBlack60: 'rgba(0, 0, 0, 0.6)',
  overlayBlack70: 'rgba(0, 0, 0, 0.7)',
  overlayBlack80: 'rgba(0, 0, 0, 0.8)',
  overlayBlack90: 'rgba(0, 0, 0, 0.9)',
  borderSubtle: 'rgba(0, 0, 0, 0.08)',
  textPrimary: '#0F172A',

  // ─── Preset Color Palette Tokens ───
  colorPaletteRed: '#EF4444',
  colorPaletteOrange: '#F97316',
  colorPaletteYellow: '#FACC15',
  colorPaletteGreen: '#10B981',
  colorPaletteCyan: '#06B6D4',
  colorPaletteBlue: '#3B82F6',
  colorPalettePurple: '#8B5CF6',
  colorPalettePink: '#EC4899',
  colorPaletteWhite: '#FFFFFF',
  colorPaletteBlack: '#0F172A',

  // ─── Domain Colors (Header Gradient Spectrum) ───
  domainColors: [
    '#4F46E5',
    '#7C3AED',
    '#2563EB',
    '#0D9488',
    '#EA580C',
    '#059669',
    '#6366F1',
    '#8B5CF6',
    '#3B82F6',
    '#D97706',
  ],
};

const DarkColors: typeof LightColors = {
  ...LightColors,
  primaryLight: '#1E1B4B',        // Deep Indigo Dark Card
  purple: '#818CF8',              // Luminous Indigo
  purpleShade50: '#1E1B4B',
  offerPurple: '#A78BFA',
  grayBorderSecondary: '#312E81',
  grayBackground: '#0F172A',      // Slate Midnight Sky
  grayText: '#94A3B8',
  grayTextWeak: '#64748B',
  greenColor: '#34D399',
  graySurface: '#1E293B',
  skyBlue: '#60A5FA',
  primaryBlack: '#F8FAFC',
  grayTextStrong: '#F8FAFC',
  dividerColor: '#1E293B',
  warningIconGold: '#FBBF24',
  shadowColorString: '#000000',
  errorColor: '#F87171',
  lightOrange: '#FDE68A',
  darkOrange: '#F59E0B',
  successGreen: '#34D399',
  purpleShade700: '#4338CA',
  greenStatus: '#064E3B',
  greenBaggageText: '#34D399',
  paleYellow: '#78350F',

  white: '#FFFFFF',
  black: '#000000',
  brandPurple: '#6366F1',
  liveGreen: '#34D399',
  firebaseOrange: '#F59E0B',
  greenA400: '#00E676',

  slate900: '#0B0813',
  slate700: '#261F38',
  slate600: '#3D3357',
  slate500: '#5C4F7F',
  slate400: '#8E82B0',
  slate200: '#C7BEDE',
  slate100: '#E4DFEE',
  slate50: '#F3F0F8',
  gray700: '#312847',
  gray300: '#C7BEDE',
  highlightYellow: '#FDE047',
  yellowHighlight: '#FFE44D',

  teal600: '#0D9488',
  cyan600: '#0891B2',
  sky600: '#0284C7',
  blue500: '#3B82F6',
  blue600: '#2563EB',
  blue700: '#1D4ED8',
  blue800: '#1E40AF',
  indigo600: '#6366F1',
  violet600: '#8B5CF6',
  violet500: '#A78BFA',
  purpleText: '#C084FC',
  pink600: '#F472B6',
  rose600: '#FB7185',
  orange600: '#FB923C',
  amber500: '#E5C158',
  amber600: '#D4AF37',
  amber700: '#C5A059',
  amber800: '#B89758',
  green500: '#22C55E',

  // ─── Telemetry Sub-Header & Performance Ribbon (Grey Theme) ───
  telemetrySubHeaderBg: '#1E293B',
  telemetrySubHeaderBorder: '#334155',
  telemetrySubHeaderGradStart: '#1E293B',
  telemetrySubHeaderGradMid: '#27354A',
  telemetrySubHeaderGradEnd: '#1E293B',
  telemetryPillBg: 'rgba(255, 255, 255, 0.08)',
  telemetryTextMuted: 'rgba(255, 255, 255, 0.70)',

  // ─── Toast / Glass / Chip Tokens (Dark) ───
  toastBg: '#1E293BEE',
  toastBorder: '#47556988',
  mintGreenBg: '#064E3B66',
  mintGreenBorder: '#05966988',
  mintGreenText: '#34D399',
  amberWarmBg: '#451A0366',
  amberWarmBorder: '#D9770688',
  amberWarmText: '#FCD34D',
  violetSoftBg: '#3B076466',
  violetSoftBorder: '#7C3AED88',
  violetSoftText: '#C084FC',
  skySoftBg: '#082F4966',
  skySoftBorder: '#0284C788',
  skySoftText: '#38BDF8',

  warningAmber: '#F59E0B',
  npmRedLight: '#FF6B6B',
  red300: '#FCA5A5',
  overlayDark: 'rgba(8, 12, 22, 0.95)',
  headerGlassDark: 'rgba(15, 23, 42, 0.9)',
  badgeGlassDark: 'rgba(15, 23, 42, 0.85)',
  borderGlassLight: 'rgba(255, 255, 255, 0.08)',
  borderGlassMedium: 'rgba(255, 255, 255, 0.15)',
  whiteAlpha06: 'rgba(255, 255, 255, 0.06)',
  whiteAlpha08: 'rgba(255, 255, 255, 0.08)',
  whiteAlpha10: 'rgba(255, 255, 255, 0.1)',
  whiteAlpha12: 'rgba(255, 255, 255, 0.12)',
  whiteAlpha15: 'rgba(255, 255, 255, 0.15)',
  whiteAlpha20: 'rgba(255, 255, 255, 0.2)',
  whiteAlpha24: 'rgba(255, 255, 255, 0.24)',
  whiteAlpha30: 'rgba(255, 255, 255, 0.3)',
  whiteAlpha33: 'rgba(255, 255, 255, 0.33)',
  whiteAlpha60: 'rgba(255, 255, 255, 0.6)',
  green600: '#16A34A',
  green700: '#15803D',
  green800: '#166534',
  emerald500: '#10B981',
  emerald600: '#059669',
  grey600: '#756C8C',
  redErrorText: '#FCA5A5',
  materialGreen: '#2E7D32',
  googleBlue: '#60A5FA',
  googleGreen: '#34D399',
  googleOrange: '#FBBF24',
  googleRed: '#F87171',
  googlePurple: '#C084FC',
  googleTeal: '#2DD4BF',

  // ─── NPM / Package / Tailwind Spectrum Helpers ───
  npmRed: '#CB3837',
  npmRedTint: '#CB383714',
  npmRedBorder: '#CB383733',
  npmDark: '#20232A',
  reactCyan: '#61DAFB',
  pink500: '#EC4899',
  pink400: '#F472B6',
  rose500: '#F43F5E',
  sky500: '#0EA5E9',
  sky400: '#38BDF8',
  indigo500: '#6366F1',
  purple500: '#8B5CF6',
  purple400: '#A78BFA',
  fuchsia500: '#D946EF',
  emerald400: '#34D399',
  amber400: '#FBBF24',
  orange500: '#F97316',
  red500: '#EF4444',
  teal400: '#2DD4BF',
  teal500: '#14B8A6',
  lime500: '#84CC16',
  yellow400: '#FACC15',
  yellow500: '#EAB308',
  slate800: '#1E293B',
  slate850: '#0F172A',

  // ─── Bundle / Extended Spectrum ───
  reanimatedNavy: '#6B8CFF',
  expoViolet: '#A78BFA',
  hermesPurple: '#9B8AFF',
  cyan500: '#22D3EE',
  indigo400: '#A5B4FC',
  indigo600Alt: '#818CF8',
  purple800: '#C084FC',
  purple900: '#A855F7',
  purple50: '#2D1B40',
  purple100: '#3B2252',
  purple200: '#4C2D6B',
  rose700: '#FB7185',
  red600: '#F87171',
  red50: '#34151B',
  red100: '#451A1A',
  amber100: '#422D0D',
  amber200: '#5C3D0F',
  amber800Warm: '#FBBF24',
  yellow200: '#5C3D0F',
  yellow800: '#FBBF24',
  green100: '#122D1B',
  emerald100: '#122D25',
  emerald700: '#34D399',
  pink100: '#341626',
  sky100: '#0C2744',
  teal100: '#134E4A',
  teal700: '#2DD4BF',
  purple700: '#8B5CF6',
  indigo50: '#1E1B3A',
  gray100: '#2A2438',
  gray200: '#362C4E',
  gray500: '#A197B8',
  gray600: '#8F8A9E',

  errorCardBg: '#34151B',
  warnCardBg: '#342B16',
  blueTintBg: '#15243B',
  purpleTintBg: '#281B3B',
  purpleTint2: '#32224A',
  roseBg: '#341626',
  roseBorder: '#782352',
  errorBorder: '#782330',
  amberBg: '#342914',
  amberBorder: '#785E22',
  emeraldBg: '#122D25',
  emeraldBorder: '#236952',
  blueBg: '#13243D',
  blueBorder: '#234978',
  greenBg: '#122D1B',
  greenBorder: '#236939',
  purpleBg: '#2D1B40',
  purpleBorder: '#62358F',
  greyBg: '#1C1628',
  greyBorder: '#362C4E',
  paperBg: '#181224',
  contentBg: '#130F1F',

  // ─── Modal, Backdrop & Overlay Tokens ───
  modalBackdrop: 'rgba(0, 0, 0, 0.85)',
  modalBackdropLight: 'rgba(0, 0, 0, 0.65)',
  modalBackdropDark: 'rgba(0, 0, 0, 0.95)',
  glassBorder: 'rgba(255, 255, 255, 0.12)',
  glassBackground: 'rgba(255, 255, 255, 0.08)',
  glassBackgroundDark: 'rgba(0, 0, 0, 0.45)',
  overlayWhite05: 'rgba(255, 255, 255, 0.05)',
  overlayWhite10: 'rgba(255, 255, 255, 0.1)',
  overlayWhite16: 'rgba(255, 255, 255, 0.16)',
  overlayWhite20: 'rgba(255, 255, 255, 0.2)',
  overlayWhite25: 'rgba(255, 255, 255, 0.25)',
  overlayWhite30: 'rgba(255, 255, 255, 0.3)',
  overlayWhite45: 'rgba(255, 255, 255, 0.45)',
  overlayBlack05: 'rgba(0, 0, 0, 0.05)',
  overlayBlack08: 'rgba(0, 0, 0, 0.08)',
  overlayBlack10: 'rgba(0, 0, 0, 0.1)',
  overlayBlack18: 'rgba(0, 0, 0, 0.18)',
  overlayBlack20: 'rgba(0, 0, 0, 0.2)',
  overlayBlack25: 'rgba(0, 0, 0, 0.25)',
  overlayBlack30: 'rgba(0, 0, 0, 0.3)',
  overlayBlack40: 'rgba(0, 0, 0, 0.4)',
  overlayBlack50: 'rgba(0, 0, 0, 0.5)',
  overlayBlack60: 'rgba(0, 0, 0, 0.6)',
  overlayBlack70: 'rgba(0, 0, 0, 0.7)',
  overlayBlack80: 'rgba(0, 0, 0, 0.8)',
  overlayBlack90: 'rgba(0, 0, 0, 0.9)',
  borderSubtle: 'rgba(255, 255, 255, 0.08)',
  textPrimary: '#FFFFFF',

  // ─── Preset Color Palette Tokens ───
  colorPaletteRed: '#EF4444',
  colorPaletteOrange: '#F97316',
  colorPaletteYellow: '#FACC15',
  colorPaletteGreen: '#10B981',
  colorPaletteCyan: '#06B6D4',
  colorPaletteBlue: '#3B82F6',
  colorPalettePurple: '#8B5CF6',
  colorPalettePink: '#EC4899',
  colorPaletteWhite: '#FFFFFF',
  colorPaletteBlack: '#0F172A',

  domainColors: [
    '#B68FE8',
    '#F472B6',
    '#E5C158',
    '#8E4DBE',
    '#2DD4BF',
    '#60A5FA',
    '#D4AF37',
    '#34D399',
    '#A78BFA',
    '#FB7185',
  ],
};

export const getThemeColors = (isDark: boolean) => {
  return isDark ? DarkColors : LightColors;
};

export const AppColors = {} as typeof LightColors;
Object.assign(AppColors, LightColors);

export const updateAppColorsTheme = (isDark: boolean) => {
  Object.assign(AppColors, getThemeColors(isDark));
};

/**
 * Configure or override specific color tokens used throughout the inspector.
 */
export const setAppColors = (customColors: Partial<typeof LightColors>): void => {
  if (!customColors || typeof customColors !== 'object') return;
  Object.assign(AppColors, customColors);
  Object.assign(LightColors, customColors);
};

export default AppColors;
